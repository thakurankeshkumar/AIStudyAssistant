...
export default function Home() {
  return (
    <main className="relative min-h-screen overflow-hidden text-[var(--foreground)]">
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_top,_rgba(255,183,77,0.14),_transparent_34%),radial-gradient(circle_at_80%_20%,_rgba(72,111,255,0.16),_transparent_28%)]" />
      <div className="relative mx-auto flex min-h-screen max-w-7xl flex-col px-6 py-6 lg:px-10">
        <header className="flex items-center justify-between gap-4 border-b border-white/10 py-4 backdrop-blur-sm">
          <div>
            <p className="text-xs uppercase tracking-[0.4em] text-[var(--muted)]">Study Assistant</p>
        ...
        <section className="grid flex-1 items-center gap-10 py-12 lg:grid-cols-[1.15fr_0.85fr] lg:py-16">
          <div className="max-w-3xl">
            <p className="inline-flex rounded-full border border-white/10 bg-white/5 px-4 py-2 text-xs uppercase tracking-[0.35em] text-[var(--muted)]">
            ...
            <div className="mt-12 grid gap-4 md:grid-cols-3">
              {features.map((feature) => (
                <article
                  key={feature.title}
                  className="rounded-2xl border border-white/10 bg-[var(--surface)] p-5 shadow-[0_20px_60px_rgba(0,0,0,0.24)] backdrop-blur"
                >
          ...
          <aside className="rounded-[2rem] border border-white/10 bg-[var(--surface-strong)] p-6 shadow-[0_24px_80px_rgba(0,0,0,0.32)] backdrop-blur md:p-8">
            <div className="flex items-center justify-between border-b border-white/10 pb-5">
              <div>
                <p className="text-xs uppercase tracking-[0.35em] text-[var(--muted)]">Flow</p>
